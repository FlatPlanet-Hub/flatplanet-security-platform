...
The current API version is `v1`, reflected in all endpoint paths (`/api/v1/...`). The current platform release is **1.9.0**.

| Version | Date | Summary |
|---|---|---|
| `1.9.0` | 2026-06-11 | Per-service tokens (Phase 1). New `service_tokens` table replaces the single shared `ServiceToken__Token` with per-caller tokens carrying explicit scopes. Admin endpoints under `/api/v1/admin/service-tokens` (PlatformOwner only) to mint, list, narrow scopes, revoke, and flush the validator cache. Tokens are formatted `fps_<service>_<43-char base64url>`, stored as SHA-256 hashes, and never retrievable after mint. Validator caches per-token claims for 60 seconds. Legacy single-token path remains active for backward compatibility until Phase 4. New audit event types: `service_token_minted`, `service_token_revoked`, `service_token_scopes_changed`, `service_token_used`, `service_token_scope_denied`, `service_token_cache_flushed`. Runbook at `docs/runbook-service-tokens.md`. |
